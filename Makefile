...
test: unittest-maintenancewindows unittest-api unittest-klm manifests test-crd generate fmt vet envtest ## Run tests.
	ENABLE_LEGACY_KYMA_DELETION=true KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) -p path)" $(GO) test -timeout 30m `go list ./tests/integration/...` -ginkgo.flake-attempts 10
