...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=controller-manager crd webhook paths="./..." paths="!./api/applyconfigurations/cert/..." output:crd:artifacts:config=config/crd/bases output:rbac:dir=config/rbac/common
...
generate-applyconfiguration: applyconfiguration-gen manifests generate-openapi-schema ## Generate applyconfiguration types.
	cd api && $(APPLYCONFIGURATION_GEN) \
		...
		./v1beta2
	cd api && $(APPLYCONFIGURATION_GEN) \
		--output-dir ./applyconfigurations/cert/gardener \
		--output-pkg github.com/kyma-project/lifecycle-manager/api/applyconfigurations/cert/gardener \
		--go-header-file ../hack/boilerplate.go.txt \
...
generate: controller-gen generate-applyconfiguration ## Generate code containing DeepCopy, DeepCopyInto, and DeepCopyObject method implementations.
	$(CONTROLLER_GEN) object:headerFile="hack/boilerplate.go.txt" paths="./..." paths="!./api/applyconfigurations/cert/..."
