...
func setupManager(flagVar *flags.FlagVar, cacheOptions cache.Options, scheme *machineryruntime.Scheme,
	...
	setupKymaReconciler(mgr, descriptorProvider, skrContextProvider, remoteClientCache, eventRecorder, flagVar, options,
		...
		kymaLookupSvc, mtEventHandlerMapFunc, mrmEventHandler)
...
func setupKymaReconciler(mgr ctrl.Manager, descriptorProvider *provider.CachedDescriptorProvider,
	...
	moduleTemplateInfoLookup := moduletemplateinfolookup.NewWithMaintenanceWindowDecorator(maintenanceWindow,
		moduletemplateinfolookup.NewLookup(mgr.GetClient()))
...
func setupManifestReconciler(mgr ctrl.Manager,
	...
	specResolver := spec.NewResolver(keychainLookupFromFlag(mgr.GetClient(), flagVar), img.NewPathExtractor(img.PullLayer))
