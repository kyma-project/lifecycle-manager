...
var _ = Describe(
	"Kyma with multiple module CRs in remote sync mode", Ordered, func() {
		...
		It(
			"CR add from client should be synced in both clusters", func() {
				By("Remote Kyma created")
				Eventually(
					KymaExists(runtimeClient, kyma.GetName(), kyma.Spec.Sync.Namespace), 30*time.Second, Interval,
				).
					Should(Succeed())
				...
				Eventually(
					UpdateRemoteModule(
						ctx, runtimeClient, kyma, []v1beta1.Module{
							*skrModuleFromClient,
						},
					), Timeout, Interval,
				).Should(Succeed())
...
var _ = Describe(
	"Kyma sync into Remote Cluster", Ordered, func() {
		...
		It(
			"Kyma CR should be synchronized in both clusters", func() {
				By("Remote Kyma created")
				Eventually(
					KymaExists(runtimeClient, kyma.GetName(), kyma.Spec.Sync.Namespace), 30*time.Second, Interval,
				).
				...
				label, err := v1.NewLabel("test", `{"foo":"bar"}`)
				Expect(err).ToNot(HaveOccurred())
				Eventually(
					ModifyModuleTemplateSpecThroughLabels(
						runtimeClient, kyma,
						v1.Labels{*label},
						true,
					), Timeout, Interval,
				...
				Eventually(
					ModuleTemplatesLabelsCountMatch(
						runtimeClient, kyma, count, true,
					), 20*time.Second, Interval,
				).Should(Succeed())
			},
		)
	},
