...
func CleanupModuleTemplateSetsForKyma(ctx context.Context, kyma *v1beta2.Kyma) func() {
	return func() {
		for _, module := range kyma.Spec.Modules {
			...
			var err error
			Eventually(func() error {
				mrmToDelete, err = testutils.GetModuleReleaseMeta(ctx, module.Name, ControlPlaneNamespace, kcpClient)
				if err != nil {
					return err
				}
				return nil
...
func createModuleTemplateSetsForKyma(moduleName string, moduleVersion, channel string) error {
	...
	mrm, err := testutils.GetModuleReleaseMeta(ctx, moduleName, ControlPlaneNamespace, kcpClient)
	...
	if mrm != nil {
		...
		for _, ch := range mrm.Spec.Channels {
			if ch.Channel == channel {
				return fmt.Errorf("channel %q already exists in ModuleReleaseMeta %q and is assigned version %q", channel, mrm.Name, ch.Version)
			}
		}
	...
	registerDescriptor(mrm.Spec.OcmComponentName, moduleVersion)
	return nil
}
