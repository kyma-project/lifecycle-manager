...
var _ = Describe("Enqueue Event from Watcher", Ordered, func() {
	...
	Context("Given SKR Cluster with TLS Secret", func() {
		...
		It("When SKR Cluster is removed", func() {
			...
			// reconciler will requeue after failing to reach the removed SKR cluster.
			// Once the access secret is deleted, the next reconcile entering processDeletion
			// skips the SKR-related cleanup tasks and eventually drops the Kyma finalizer.
