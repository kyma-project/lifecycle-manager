...
test-run: log-tool-versions
	...
	@echo "::group::E2E test: Non Blocking Module Deletion With Only Default CR"
	@export PATH=$(LOCALBIN):$$PATH
	@pushd $(E2E_TESTS_DIR) > /dev/null
	set +e; $(GO) test -timeout 20m -ginkgo.v -ginkgo.focus "Non Blocking Module Deletion With Only Default CR"; status=$$?; set -e
	@popd > /dev/null
	@echo "::endgroup::"
	exit $${status}
