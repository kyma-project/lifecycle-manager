...
test-run: log-tool-versions
	...
	@echo "::group::E2E test: KCP Kyma Module status on SKR connection lost"
	@export PATH=$(LOCALBIN):$$PATH
	@pushd $(E2E_TESTS_DIR) > /dev/null
	set +e; $(GO) test -timeout 20m -ginkgo.v -ginkgo.focus "KCP Kyma Module status on SKR connection lost"; status=$$?; set -e
	@popd > /dev/null
	@echo "::endgroup::"
	exit $${status}
