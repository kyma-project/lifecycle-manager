...
test-run: log-tool-versions
	...
	@echo "::group::E2E test: Blocking Module Deletion With Module CRs in Different Namespaces with Ignore Policy"
	@export PATH=$(LOCALBIN):$$PATH
	@pushd $(E2E_TESTS_DIR) > /dev/null
	set +e; $(GO) test -timeout 20m -ginkgo.v -ginkgo.focus "Blocking Module Deletion With Module CRs in Different Namespaces with Ignore Policy"; status=$$?; set -e
	@popd > /dev/null
	@echo "::endgroup::"
	exit $${status}
