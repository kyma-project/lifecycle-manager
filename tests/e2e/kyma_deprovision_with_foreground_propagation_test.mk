...
test-run: log-tool-versions
	...
	@echo "::group::E2E test: KCP Kyma CR Deprovision With Foreground Propagation After SKR Cluster Removal"
	@export PATH=$(LOCALBIN):$$PATH
	@pushd $(E2E_TESTS_DIR) > /dev/null
	set +e; $(GO) test -timeout 20m -ginkgo.v -ginkgo.focus "KCP Kyma CR Deprovision With Foreground Propagation After SKR Cluster Removal"; status=$$?; set -e
	@popd > /dev/null
	@echo "::endgroup::"
	exit $${status}
