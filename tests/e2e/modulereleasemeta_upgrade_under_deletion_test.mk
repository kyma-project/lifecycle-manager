...
test-run: log-tool-versions
	...
	@echo "::group::E2E test: Kyma Module with ModuleReleaseMeta Upgrade Under Deletion"
	@export PATH=$(LOCALBIN):$$PATH
	@pushd $(E2E_TESTS_DIR) > /dev/null
	set +e; $(GO) test -timeout 20m -ginkgo.v -ginkgo.focus "Kyma Module with ModuleReleaseMeta Upgrade Under Deletion"; status=$$?; set -e
	@popd > /dev/null
	@echo "::endgroup::"
	exit $${status}
