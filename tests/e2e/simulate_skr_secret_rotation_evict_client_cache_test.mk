...
test-run: log-tool-versions
	...
	@echo "::group::E2E test: SKR client cache get evicted due to connection error caused by secret rotation"
	@export PATH=$(LOCALBIN):$$PATH
	@pushd $(E2E_TESTS_DIR) > /dev/null
	set +e; $(GO) test -timeout 20m -ginkgo.v -ginkgo.focus "SKR client cache get evicted due to connection error caused by secret rotation"; status=$$?; set -e
	@popd > /dev/null
	@echo "::endgroup::"
	exit $${status}
