...
func (s ByModuleReleaseMetaStrategy) Lookup(ctx context.Context,
	...
	//[Review Note]
	//the original name: "desiredModuleVersion" is misleading: it would be "desired", if user provides it.
	//here we resolve the version using data from moduleReleaseMeta, so "resolvedModuleVersion" seems to be more appropriate.
