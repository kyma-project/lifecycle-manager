...
| `oci-registry-cred-secret`    | string   | ""                                                                   | Allows to configure the name of the Secret containing the credentials of the OCI registry storing the OCM component versions of modules. Must not be set together with `--oci-registry-host`. The Secret must be of type `kubernetes.io/dockerconfigjson`.              |
