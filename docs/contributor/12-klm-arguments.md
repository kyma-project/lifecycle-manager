...
| `k8s-client-qps`   | int   | 50           | Maximum queries per second (QPS) limit for the SKR Kubernetes client. Controls how many requests can be made to the Kubernetes API server per second in the steady state |
| `k8s-client-burst` | int   | 100           | Maximum burst size for throttling SKR Kubernetes API requests. Allows temporarily exceeding the QPS limit when there are sudden spikes in request volume                 |
