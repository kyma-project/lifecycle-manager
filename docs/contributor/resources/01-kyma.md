...
* `operator.kyma-project.io/skip-reconciliation`: A label that can be used with the value `true` to completely disable reconciliation for an active Kyma CR. It can also be used in the Manifest CR to disable a specific module. This disables all reconciliations for the target Kyma CR or Manifest CR while the Kyma CR is not being deleted. Note that this label does **not** affect the Kyma CR deletion process — once a Kyma CR is marked for deletion (DeletionTimestamp is set), the reconciler will always proceed with the deletion flow regardless of this label. Also note that even if reconciliation is disabled for the Kyma CR, the Manifest CR in a Kyma CR can still be reconciled normally if not adjusted to have the label set as well.
