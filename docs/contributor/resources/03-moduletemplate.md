...
The recommended workflow for creating ModuleTemplates uses [modulectl](https://github.com/kyma-project/modulectl/tree/main) in combination with [OCM CLI](https://github.com/open-component-model/ocm/tree/main/cmds/ocm):

1. **Configure OCM Registry Access**: Create an OCM configuration file to define registry aliases and enable insecure HTTP communication for local/test registries:

   For local registries (e.g., `localhost:5111`):
   ```yaml
   # ocm-config.yaml
     ...
     - type: ocm.config.ocm.software
       aliases:
         registry.example.com:5001:
           type: OCIRegistry
           baseUrl: http://registry.example.com:5001
     - type: oci.config.ocm.software
       aliases:
         registry.example.com:5001:
           type: OCIRegistry
   ...
   > **Note**: Using `http://` (insecure) is recommended for local development and testing. For production environments, use `https://` with proper TLS certificates.

2. **Generate Component Constructor**: Use `modulectl create` with the `--disable-ocm-registry-push` flag to generate the component constructor file without pushing to a registry:
   ```bash
   modulectl create \
...
3. **Create Component Version**: Use OCM CLI with the config file to add the component version to a Common Transport Format (CTF) archive:
   ```bash
   ocm --config ocm-config.yaml add componentversions \
     --create \
     --file ./component-ctf \
...
4. **Transfer to Registry**: Use OCM CLI to transfer the component version to your OCM registry:
   ```bash
   ocm --config ocm-config.yaml transfer ctf \
     --overwrite \
     --no-update \
     ./component-ctf \
     http://localhost:5111
...
5. **Apply ModuleTemplate**: The `template.yaml` file generated by modulectl contains the ModuleTemplate CR with the component descriptor. Apply it to your cluster:
   ```bash
   kubectl apply -f template.yaml
   ```

This workflow ensures that:
- OCM CLI can communicate with local/insecure registries using HTTP
- Registry credentials are properly configured for private registries
- Component descriptors are properly constructed with all required resources (images, manifests, default CRs)
- Component versions are stored in OCM-compliant registries
- ModuleTemplates reference the correct component versions with valid descriptors

For more details on the modulectl configuration, see the [modulectl documentation](https://github.com/kyma-project/modulectl/tree/main).
