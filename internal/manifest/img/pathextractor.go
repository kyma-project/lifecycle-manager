...
func (p PathExtractor) GetPathForFetchedLayer(ctx context.Context,
	...
	//Review Note: previously the source of imageSpec.Repo was a component descriptor, which alredy contained the `component-descriptors` suffix.
	//Now the source for repository is the user-provided configuration value that specifies just the host.
	//Therefore we need to add the `component-descriptors` part manually here.
	imageRef := fmt.Sprintf("%s/%s/%s@%s", imageSpec.Repo, componentmapping.ComponentDescriptorNamespace, imageSpec.Name, imageSpec.Ref)
