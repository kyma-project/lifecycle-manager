...
import (
	...
	"github.com/kyma-project/lifecycle-manager/internal/descriptor/type"
)
...
func TestGet_ForCacheWithAnEntry_ReturnsAnEntry(t *testing.T) {
	...
	desc1 := &_type.Descriptor{ComponentDescriptor: ocmDesc1}
...
func TestGet_ForCacheWithOverwrittenEntry_ReturnsNewEntry(t *testing.T) {
	descriptorCache := cache.NewDescriptorCache()
	originalKey, originalValue := cache.DescriptorKey("key 1"), &_type.Descriptor{
	...
	newKey, newValue := cache.DescriptorKey("key 2"), &_type.Descriptor{
		ComponentDescriptor: &compdesc.ComponentDescriptor{
			ComponentSpec: compdesc.ComponentSpec{
...
func assertDescriptorEqual(t *testing.T, expected, actual *_type.Descriptor) {
	t.Helper()
...
func assertDescriptorNotEqual(t *testing.T, expected, actual *_type.Descriptor) {
	t.Helper()
	if expected.Name == actual.Name {
