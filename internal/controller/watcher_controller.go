...
func (r *WatcherReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !watcherObj.DeletionTimestamp.IsZero() && watcherObj.Status.State != StateDeleting {
		return r.updateWatcherState(ctx, watcherObj, StateDeleting, nil)
	}
...
func (r *WatcherReconciler) stateHandling(ctx context.Context, watcherCR *v1beta2.Watcher) (ctrl.Result, error) {
	...
	case "":
		return r.updateWatcherState(ctx, watcherCR, StateProcessing, nil)
...
func (r *WatcherReconciler) handleDeletingState(ctx context.Context, watcherCR *v1beta2.Watcher) (ctrl.Result, error) {
	...
	if err != nil {
		...
		return r.updateWatcherState(ctx, watcherCR, StateError, vsConfigDelErr)
	}
	finalizerRemoved := controllerutil.RemoveFinalizer(watcherCR, watcherFinalizer)
	if !finalizerRemoved {
		return r.updateWatcherState(ctx, watcherCR, StateError, errRemovingFinalizer)
	}
...
func (r *WatcherReconciler) handleProcessingState(ctx context.Context,
	...
	if err != nil {
		return r.updateWatcherState(ctx, watcherCR, StateError, err)
	}
	...
	if util.IsNotFound(err) {
		...
		if err != nil {
			...
			return r.updateWatcherState(ctx, watcherCR, StateError, vsCreateErr)
		}
		return r.updateWatcherState(ctx, watcherCR, StateReady, nil)
	}
	...
	if err != nil {
		...
		return r.updateWatcherState(ctx, watcherCR, StateError, vsUpdateErr)
	}
	return r.updateWatcherState(ctx, watcherCR, StateReady, nil)
}
