...
DEFAULT_CR_FILE="config/samples/default-sample-cr.yaml"

# Manipulate manifest to introduce new CRD version
yq eval '(.items[] | select(.kind == "CustomResourceDefinition" and .metadata.name == "samples.operator.kyma-project.io") | .spec.versions) += [{"name": "v1beta1", "served": true, "storage": true, "schema": (.spec.versions[0].schema)}]' -i "${MANIFEST_FILE}"
yq eval '(.items[] | select(.kind == "CustomResourceDefinition" and .metadata.name == "samples.operator.kyma-project.io") | .spec.versions[] | select(.name == "v1alpha1") | .storage) = false' -i "${MANIFEST_FILE}"

# Update default CR to new CRD version
yq eval '.apiVersion = "operator.kyma-project.io/v1beta1"' -i "${DEFAULT_CR_FILE}"
